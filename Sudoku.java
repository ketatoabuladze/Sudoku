...
public class Sudoku {
    ...
    //write code here
    // Check if the location is updatable (spot is empty)
    if (!isLocationUpdatable(i, j)) {
        return false; // The spot is not empty (0), can't update it
    }

    //Check if row and column indices are valid (between 0 and 8 inclusive)
    if (i < 0 || i >= N || j < 0 || j >= N) {
    ...
    if (num < 1 || num > 9) {
        return false; // Invalid number, must be between 1 and 9
    }

    //Check if the number is safe to place in the spot
    if (!CheckIfSafe(i, j, num)) {
        return false; // Number violates Sudoku rules (row, column, or box conflict)
    }

    // If all checks passed, update the board
    mat[i][j] = num;
